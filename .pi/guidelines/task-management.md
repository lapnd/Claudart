---
paths: ["**/*"]
description: How agents create, maintain, resume, and complete persistent implementation plans stored in `.pi/tasks/`. Replaces session-only plan mode with cross-session task documents.
when_to_use: Whenever the user invokes `/skill:pi-plan`, when a task file is open or referenced, or when resuming work that may have an active task in `.pi/tasks/`.
tags: [tasks, planning, persistence, cross-session]
---

# Task Management

Plans live as markdown documents in `.pi/tasks/`, not in session memory. One task per file. The file is self-contained — reading it alone must be enough to resume work in a future session, even after intervening commits.
...
Mission-scale work runs one layer up, in `.pi/specs/` (see `spec-workflow.md`), and **supersedes this guideline within its scope**: an approved spec's standing approval replaces the per-task approval and review gates below, and a spec executor never creates task files. Never run both layers over the same work.
...
- **`index.md` is a dashboard**, maintained by `/skill:pi-plan` and `/skill:pi-checkpoint`. Task files are the source of truth; `index.md` is a convenience cache.
...
- **Decision** (YYYY-MM-DD HH:MMZ, pi): <what was chosen>.
...
- **`done`**: completed AND user-confirmed. Move file to `tasks/done/`. Append one line to `.pi/JOURNAL.md`.
...
- **Do NOT modify any code file.** No writes against anything outside `.pi/tasks/`.
...
2. Bump `updated:`.
3. Move the file to `.pi/tasks/done/`.
4. Append one line to `.pi/JOURNAL.md`:
   ```
   YYYY-MM-DD | completed | <slug> — <one-line outcome>, see tasks/done/<filename>
   ```
5. Update `.pi/tasks/index.md`: remove from Active, add to Recently Done.
6. If a recurring pattern emerged, propose `/skill:pi-learn` to graduate it into a guideline.
...
```markdown
<!-- .pi/tasks/index.md — dashboard of task documents. Maintained by /skill:pi-plan and /skill:pi-checkpoint. -->
...
Canonical numbers for flagging stalled tasks. `/skill:pi-start` surfaces them, `/skill:pi-checkpoint` acts on them, `/skill:pi-doctor` audits them — none of those files redefine the numbers.
...
- **CONTEXT.md** holds two things: (a) a one-line pointer to the currently-focused task (`Working task \`<slug>\` (see .pi/tasks/<file>)`) so `/skill:pi-start`sees task work at a glance, and (b) ad-hoc work the user asked for **without** a`/skill:pi-plan` — quick fixes, transient tweaks, mid-flight pivots that don't justify a full task document.
...
- Copying a task's body (Steps / Decisions / Surprises / Memory Hints) into `.pi/CONTEXT.md`. CONTEXT may _reference_ the active task by slug + path, but must never duplicate its content.
- Auto-loading task files via `PI.md`. Task files are working documents, not always-loaded guidelines.
