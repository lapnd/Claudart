---
paths: ["**/*"]
description: Dated mission-scale spec workspaces in `.pi/specs/` with `done/` archives — a POC-frozen SPEC plus a decision-complete ROADMAP that any later session (often a cheaper model) executes autonomously until final review, with self-QA, circuit breakers, and session rotation.
when_to_use: Whenever the user invokes `/skill:pi-spec` or `/skill:pi-spec-run`, when a spec folder under `.pi/specs/` is open or referenced, or when resuming mission-scale work that spans many sessions.
tags: [specs, loop-engineering, autonomy, cross-session, missions]
---

# Spec Workflow (Loop Engineering)

A **spec** is a mission: work too large for one task file — a whole game, a feature system, a client-demo POC. While active, it lives as a dated folder in `.pi/specs/YYYY-MM-DD-<slug>/` written once by an expensive planning session (`/skill:pi-spec`), then executed to the final-review gate across many sessions by `/skill:pi-spec-run` — often on a cheaper model — **without per-task human approval**. Completed and cancelled missions are archived under `.pi/specs/done/YYYY-MM-DD-<slug>/`. The folder, not any session, is the source of truth; every iteration assumes total amnesia and re-orients from files.

Missions sit **above** the task layer (`task-management.md`): a spec supersedes `/skill:pi-plan` for its scope, and its executor never creates `.pi/tasks/` files. Use `/skill:pi-plan` for a single feature or fix; use `/skill:pi-spec` when the deliverable is a demoable whole.
...
- **Resolving a spec**: `/skill:pi-spec-run <arg>` accepts either a full folder id (`YYYY-MM-DD-<slug>`) or the short slug. Match active top-level folders first; if a short slug matches more than one active folder, ask the user to choose the dated folder. If the match exists only under `done/`, report its archived status and do not run it.
- **`SPEC.md` holds intent, `ROADMAP.md` holds the plan, `NOTES.md` holds mission working knowledge/candidates, `LEDGER.md` holds the proof.** Canonical durable descriptive facts live under `.pi/knowledge/` only after the capture gate passes.
...
- Evidence: bash .pi/scripts/disjoint.sh . 'a:src/api/\*\*' 'b:src/cli/\*\*' → disjoint
```
...
- **Seeded by `/skill:pi-spec`** from planning-time exploration; **grown by the executor** whenever a finding or mid-flight decision matters to this mission. Route by kind: evidence → LEDGER; WIP, proposals, mission state, and uncertain discoveries → NOTES.
- **Scope-check as you write**: a candidate that may outlive the mission gets flagged in place — `→ graduate: knowledge/` for descriptive facts, `→ graduate: /skill:pi-learn` for recurring behavior. Checkpoint bulk-evaluates flags at rotations and close.
...
NOTES remains the default home for mission-local and uncertain material. A direct knowledge mutation is allowed only when the full capture gate and one of `knowledge-management.md`'s immediate-promotion triggers pass. Scope may be narrow. Read that rule in full, patch the existing owner first, update its reachable route atomically, and run `bash .pi/scripts/knowledge-check.sh --root .`. This exception does not authorize implementation code during a read-only spec state.
...
- **On yes**: append a `rotation-checkpoint` LEDGER entry (one-line state + exact next task), bump `updated:`, then run the `/skill:pi-checkpoint` flow — it syncs the specs INDEX, refreshes CONTEXT's spec pointer, and collects NOTES' `→ graduate:` flags — and tell the user: open a fresh session, orient with `/skill:pi-start`, and run `/skill:pi-spec-run <slug>`.
- **On no**: continue the loop.
- Do **not** write `.pi/HANDOFF.md` for spec work — SPEC + ROADMAP + NOTES + LEDGER _are_ the baton, and the loop is amnesia-first by design.
...
5. User confirms → `done`: flip `status: done`, append one line to `.pi/JOURNAL.md` (`YYYY-MM-DD | completed | spec <slug> — <one-line outcome>, see specs/done/YYYY-MM-DD-<slug>/SPEC.md`), and sweep `NOTES.md` before shelving — evaluate remaining `→ graduate: knowledge/` candidates against the full capture gate, write eligible facts under the atomic owner/map contract, run the checker after mutations, and leave uncertainty in NOTES; propose `/skill:pi-learn` for behavioral lessons. Then move the entire folder from `.pi/specs/YYYY-MM-DD-<slug>/` to `.pi/specs/done/YYYY-MM-DD-<slug>/` and sync INDEX.
...
- **Material or ambiguous amendment** — anything that cannot satisfy the bounded-patch test remains under the read-only lock. Surface the scope delta and ask whether to amend the SPEC. On explicit yes, flip to `drafting`, sync INDEX, and return control to `/skill:pi-spec` for amendment and renewed approval.
...
ready ──(/skill:pi-spec-run picks it up)──▶ running
...
- `drafting` / `poc-review`: `/skill:pi-spec` owns these; **no implementation code may be written** — only spec-folder files and artifacts.
- `ready`: approved, not yet started. `running`: the loop is live. Both belong to `/skill:pi-spec-run`.
...
```markdown
<!-- .pi/specs/INDEX.md — registry of spec missions. Maintained by /skill:pi-spec, /skill:pi-spec-run, /skill:pi-checkpoint. -->
...
SPEC frontmatter is the source of truth; INDEX is a cache. Active lists status ∈ {drafting, poc-review, ready, running, blocked, awaiting-final-review} from top-level dated folders — append ` ⏳ awaiting your review` to `poc-review` and `awaiting-final-review` lines. Done lists status ∈ {done, cancelled} from `.pi/specs/done/`. A top-level folder whose SPEC status is `done` or `cancelled` is stale and must be archived before INDEX is rewritten. Spec folders are never deleted.

## Relationship to the Rest of CLAUDART

- **Tasks**: a spec replaces `/skill:pi-plan` for its scope. Never mirror roadmap tasks into `.pi/tasks/`; never run both layers over the same work.
- **CONTEXT.md**: may carry one pointer line (`Running spec \`<slug>\` (see .pi/specs/YYYY-MM-DD-<slug>/SPEC.md)`); never absorbs spec content.
- **`/skill:pi-start`**: surfaces Active specs from INDEX and directs the user to `/skill:pi-spec`, `/skill:pi-spec-run`, or final verification according to status.
- **`/skill:pi-checkpoint`**: syncs INDEX from SPEC frontmatter, same as it syncs `tasks/index.md`.
- **knowledge/**: read at `/skill:pi-spec` planning time using map-first bounded routing. Mid-run discoveries default to NOTES candidates. The executor may write an eligible fact directly only under the knowledge-maintenance exception; otherwise checkpoint bulk-evaluates flags at rotation and close.
- **guidelines/**: the executor obeys them but never edits them mid-loop. A recurring behavioral lesson (the same correction needed twice) gets a NOTES flag `→ graduate: /skill:pi-learn`, proposed at rotation or mission close — guideline changes stay user-gated.
