...
- **Concrete Steps**: ordered checklist. Each step is one self-contained action with target file, expected outcome, and a `(verify: <observable check>)`. Steps should be small enough that completing one is a meaningful save point, and written at plan altitude — decisions and verification, never code. Append `(tier: fast)` or `(tier: strong)` when a step deviates from the default `standard` tier, per `agent-delegation.md` → Model & Effort Routing — the executing session uses these to route the work and its subagents.
