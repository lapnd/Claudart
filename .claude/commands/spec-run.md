...
- Compute the runnable **set** before selecting — `bash .claude/scripts/spec-check.sh next --root . --layer claude` reports every non-done task as `runnable`, `blocked`, `waiting: <node>`, or `unmodelled`, so nothing is silently in or out. Never select a task marked `⚠ blocked` or an invalid legacy struck-unticked row. Derive the _how_ yourself from the roadmap's decisions — the plan carries decisions and `verify:`, not solutions.
- Taking fewer than the runnable set requires a `wave-selected` LEDGER entry naming each excluded task and its excluding rule (1 overlap / 2 output dependency / 3 ordering), per the skill's Loop step 2. Measure rule 1 with `bash .claude/scripts/disjoint.sh <repo> 'a:<glob>' 'b:<glob>'`; it answers file overlap only and is never permission to fan out. Serial is a legitimate choice and an unrecorded serial default is not.
