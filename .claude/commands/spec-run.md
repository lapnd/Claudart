...
The loop is model-agnostic: run it on a cheap model for routine execution, and run the _same command_ from a stronger session to punch through a `blocked` spec — escalation is not a different workflow (see the rule file's unblock mode). SPEC frontmatter `executor-tier:` records the cheapest recommended session tier (see `agent-delegation.md` → Model & Effort Routing); if this session is below it, say so before starting.
...
If the spec folder contains `architecture.yaml`, the mission is **graph-driven**: finish orientation with `bash .claude/scripts/claudart-graph.sh status --dir <spec>` and `lint --dir <spec>`. A lint exit `1` (findings) or `2` (manifest/ROADMAP rejected) blocks the iteration — fix it before scheduling anything; never schedule on a dirty graph.
...
- Delegate per `agent-delegation.md` whenever work genuinely parallelizes; roadmap-marked waves are recorded strategy, not an authorization gate — re-derive runnable waves each iteration with the disjointness test rather than trusting a stale marking. Worker prompts carry the roadmap task text and relevant SPEC lines verbatim. Re-verify every worker result yourself before ticking. For a coding wave, fan out with `isolation: worktree` and run the full Worktree Lifecycle for Parallel Coding Work — merge one branch at a time, re-verify on the merged state, confirm the merge landed, and only then delete the worktree; push stays governed by `git-commits.md` and is never auto-granted by fanning out.
...
- A second `validation-failed` on the same task in a session below the strong tier is an escalation signal: before a third attempt, offer rotation to a stronger session — escalation is this same command from that session — unless the next attempt is materially different for reasons unrelated to reasoning depth.
...
#### Graph-driven missions (`architecture.yaml` present)

The rule file's **Graph-driven iteration** section is binding here; a mission with no manifest keeps the bullets above unchanged. In practice:

- `bash .claude/scripts/claudart-graph.sh next --dir <spec> --root <repo root>` is the runnable set — it replaces the by-hand disjointness call. Its `READY tests to write` go first (they must be observed RED), then its `READY implementations`. A `MERGE RISK a <-> b [...]` line means run both, each in its own git worktree, and merge in the order `schedule` prints; it never shrinks the wave. An `EXCLUSIVE` node runs alone at the first quiet moment.
- A worker receives only `brief <node> --dir <spec>` — never the ROADMAP, never the manifest — and writes nothing under the spec folder (no ticks, no LEDGER, no graph events, and never its own `done`). It returns files changed, the verify command it ran, its exit, and residual risk; you still append `task-started`/`delegated` to the LEDGER.
- You, the orchestrator, record the outcome after merging the branch (or when working solo): `event <node> done --run --dir <spec> --root <repo root> [--agent <tier>] [--model <m> --tokens-in N --tokens-out N --duration S]`. `--run` re-executes the node's `verify:` and records the exit **you** observed; a claimed exit is rejected (`CLAIMED-EXIT-REJECTED`) and a failing verify is recorded as `validation-failed`, so nothing is ticked. A test node takes `event <test> red --run` first (must exit nonzero); its green is written automatically when the implementation it proves goes `done`, and `green` is impossible without a prior `red`. Honor any `RE-VERIFY on the integrated tree: …` line by re-running `event <node> done --run` for each node named. Tick the checkbox and write `task-completed` only after the event landed, citing its command, exit, and `graph/evidence/<node>.log`.
- A conflict between two worktree branches is the synthesised `merge/<a>+<b>` node that `schedule` names: resolve it as a `tier: strong` node whose verify is both nodes' test suites green after the merge, record it with `event merge/<a>+<b> done --run`, append a roadmap task line for it under the append-missing-work rule and log `replanned` — never discard one side.

### Step 3 — Phase boundary

Run the phase validation. On PASS, tick the SPEC scenarios it proves and append `phase-validated`. On FAIL, follow the rule's convergence flow: keep the phase open, append `validation-failed`, update Current Acceptance Delta, and reopen responsible work under the ROADMAP disposition contract; do not create fix/replay bookkeeping pairs. Then **apply the rotation policy** per the rule file (next paragraph): under `auto`, rotate; under `offer`, report `n/m` tasks + current phase + Current Acceptance Delta and ask "checkpoint and rotate, or continue?". On rotate: `rotation-checkpoint` LEDGER entry with the exact next task, then the `/checkpoint` flow (specs INDEX sync + NOTES `→ graduate:` flags collected), then tell the user to open a fresh session, `/start`, then `/spec-run <slug>`. Also offer rotation mid-phase after any compaction — finish the in-flight task first.

For a graph-driven mission, also run `bash .claude/scripts/claudart-graph.sh gate --dir <spec> --violations <measured count, or 0 while no extractor exists> [--mutation <score>] --check-evidence` — the phase closes only on `GATE OPEN` for that phase's nodes, and the gate lists exactly what is missing — plus `progress --fail-on-regression`, where a test that was green and is red again is a regression that reopens work.

Rotation behaviour follows SPEC frontmatter `rotation:` (missing → `auto`). Under `auto` you do not ask: finish the in-flight task, append `rotation-checkpoint` (state + exact next task), run the `/checkpoint` flow, call `bash .claude/scripts/claudart-rotate.sh <slug>` — it launches the successor headlessly (`claude -p "/start then run /spec-run <slug>" --permission-mode acceptEdits --allowedTools <toolkit>`), writes its transcript under `<spec>/artifacts/rotations/`, and appends the log path to the LEDGER — then **stop** this session: report the log path and do nothing else. If the script refuses (spec not `ready`/`running`, a rotation log younger than 60 s, ceiling `CLAUDART_MAX_ROTATIONS` reached, `claude` absent), report the refusal verbatim and still stop. It never bypasses permissions by itself; under `offer` you ask the prompt above and wait for the answer. If the script is absent, behave as `offer` and say so.

### Step 4 — Final gate

For a graph-driven mission the final gate is `bash .claude/scripts/claudart-graph.sh gate --dir <spec> --violations <count or 0> [--mutation <score>] --check-evidence` exiting `0` (`GATE OPEN`) **plus** the scenario evidence described below — neither substitutes for the other.
