...
- optional specialized agents in `.claude/agents/`, read-only by default and write-capable only when the declared purpose explicitly requires implementation.
...
- Keep explorers and review-only/audit-only agents read-only. Allow `Edit` only when the declared purpose explicitly requires implementation, as with a worker or refiner.
- Confirm every write-capable agent defines ownership boundaries, protects unrelated user work, validates its own changes, and states that it must not revert edits made by others in parallel.
