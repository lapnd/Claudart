...
4. **Route durable content out FIRST.** The baton holds conversational residue only — content with no durable home. Before writing it: task/spec state, WIP, proposals, and uncertain candidates → the owning task/spec artifact; recurring behavior → name it as a `/learn` candidate; descriptive facts → promote only when they pass `.claude/skills/knowledge-management/SKILL.md`. Scope may be local. Whatever you routed out does NOT also go into the baton.
