---
description: Export a portable CLAUDART bundle — the project .claude/ layer plus this project's Claude Code session history — for moving to another machine or grafting into another project.
---

The user's request after `/backup` is producing a portable bundle of this project's CLAUDART content: session transcripts, per-project memory, knowledge, rules, tasks, specs, and prompt history. Bundle format, path-token handling, and every safety rail (allow-list collection, secret scan, atomic write) live in `.claude/scripts/claudart-backup.sh` — this command drives that script, it does not reimplement it.

1. Confirm the destination with the user if not stated (`--out DIR`, must not already exist).
2. Run `bash .claude/scripts/claudart-backup.sh --out <dir> [options]` — see `--help` for the full flag reference (`--sessions all|recent|none`, `--history yes|no`, `--project-scope all|graft`, `--alias-root`, `--archive`, `--dry-run`).
...
5. Point the user at `.claude/scripts/claudart-restore.sh` (or `/restore`) as the next step, and mention the bundle's `README.txt` documents exactly what is and isn't inside.
