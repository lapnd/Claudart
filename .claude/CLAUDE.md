...
- `/migrate <mission>` creates a cross-runtime port mission (Python→Go, NiceGUI→Vue, any language/runtime/UI-framework change) — pins the source baseline, decomposes a UI-fused source, freezes the API seam and this project's translation rules, and proves parity by running both stacks; executed by `/spec-run`.
- `/graph [slug]` drives the deterministic development-graph engine (`.claude/scripts/claudart-graph.sh`) — lint, schedule, next, brief, event, gate — over a spec's `architecture.yaml` + graph-format `ROADMAP.md`; see `docs/GRAPH.md`.
...
See @.claude/rules/constitution.md for the decision-priority ladder and sixteen Golden Rules that sit above every other rule.
See @.claude/rules/ai-behavior.md for universal AI behavior guidelines.

The workflow rules below are deliberately **not auto-imported** (token hygiene). Read the rule file when its trigger fires, **before acting under it**. Until read, the digest after each trigger is binding:
...
Reference skills live in `.claude/skills/` (`python-to-go-idioms`, `nicegui-to-vue`). They are load-on-demand dictionaries, never auto-imported — read one when its construct is in front of you, and ground its rows in the project's own `artifacts/translation-rules.md` rather than citing the generic table.
