#!/usr/bin/env bash
# claudart-law.sh — entry point for the CLAUDART law engine (composition root).
#
...
if ! command -v python3 >/dev/null 2>&1; then
  printf 'claudart-law: python3 is required (stdlib only, no packages) and was not found on PATH\n' >&2
  exit 2
fi

PYTHONPATH="$HERE${PYTHONPATH:+:$PYTHONPATH}" exec python3 "$HERE/law/adapters/cli.py" "$@"
