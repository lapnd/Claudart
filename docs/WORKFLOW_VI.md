...
| `/migrate <mission>`  | —                           | Mission port cross-runtime (Python→Go, NiceGUI→Vue) - ghim baseline nguồn, đóng băng API seam và bảng translation-rules riêng của dự án, decompose callback UI-fused, chứng minh parity bằng cách chạy song song hai stack ([hướng dẫn](MIGRATE_VI.md)) |
| `/graph [slug]`       | `$codex-graph [slug]`       | Điều khiển engine development-graph deterministic (lint, schedule, next, brief, event, gate) cho `architecture.yaml` + ROADMAP dạng graph của một spec ([hướng dẫn](GRAPH_VI.md))                                                                       |
...
| `/prove <change>`     | `$codex-prove <change>`     | Vòng lặp evidence-first cho một thay đổi - khai báo tier theo blast radius, quan sát test đỏ trước khi implement (`red-verified`), chạy gauntlet (gate changed-line coverage, mutation, property test), báo cáo bằng số liệu thay vì tính từ            |
