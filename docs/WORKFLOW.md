...
| `/migrate <mission>`  | —                           | Cross-runtime port mission (Python→Go, NiceGUI→Vue) — pins the source baseline, freezes the API seam and a project-specific translation-rules table, decomposes UI-fused callbacks, and proves parity by running both stacks ([guide](MIGRATE.md))          |
| `/graph [slug]`       | `$codex-graph [slug]`       | Drives the deterministic development-graph engine (lint, schedule, next, brief, event, gate) for a spec's `architecture.yaml` + graph-format ROADMAP ([guide](GRAPH.md))                                                                                    |
| `/refactor <mission>` | `$codex-refactor <mission>` | Behavior-preserving refactor/migration mission — pins a baseline, writes a behavior contract + blast radius from the pre-change code, derives equivalence-proof scenarios executed by the spec runner                                                       |
| `/project-discovery`  | `$codex-project-discovery`  | Interview-first planning — turns rough ideas into project docs before any code                                                                                                                                                                              |
| `/prove <change>`     | `$codex-prove <change>`     | Evidence-first loop for one change — declares a tier by blast radius, observes each test failing before implementing (`red-verified`), runs the gauntlet (changed-line coverage gate, mutation, property tests), and reports numbers rather than adjectives |
