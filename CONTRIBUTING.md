...
- **Invocation forms differ per harness and are not interchangeable.** Codex uses `$codex-<name>`; dsh invokes a user-invocable skill with `/<name>`; Pi registers skills as `/skill:<name>`. A layer forked from another must have its command form re-checked against that harness's documentation, not carried over.
