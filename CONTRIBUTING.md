...
- `.claude/scripts/claudart-backup.sh` / `claudart-restore.sh` and their `.codex/scripts/` twins: byte-identical copies of the portability engines behind `/backup` and `/restore`. Change and test them as one unit (`tests/portability/run.sh`); its fixtures are built inline at runtime and must stay synthetic.
