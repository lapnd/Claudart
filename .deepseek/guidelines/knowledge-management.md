---
paths: ["**/*"]
description: Canonical contract for routing, capturing, validating, and maintaining durable descriptive project knowledge under `.deepseek/knowledge/`.
when_to_use: When a task needs project knowledge beyond the root map, when the user asks to update or search prior project knowledge, or when a workflow audits or mutates `.deepseek/knowledge/`.
...
1. Read `.deepseek/knowledge/INDEX.md`.
...
1. Confirm `.deepseek/scripts/knowledge-check.sh` exists before writing; missing checker is a High-severity installation problem.
