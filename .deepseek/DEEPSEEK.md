...
This repository contains CLAUDART, a markdown-based operating layer for AI coding agents. This file is the DeepSeek layer's instructions index.

DeepSeek Harness (`dsh`) does not auto-load this file by name. It loads `AGENTS.md` and `CLAUDE.md` (plus `AGENTS.local.md` / `CLAUDE.local.md` overlays), from `$DSH_HOME/AGENTS.md` and then every directory from the project root — the nearest ancestor containing `.git` — down to the session working directory. The project's root `AGENTS.md` carries a CLAUDART route line pointing here; follow it, then follow what this file routes to. A nested `AGENTS.md` deeper in the tree is picked up after a successful `read`, `write`, or `edit` reaches that directory.

Skills are discovered from `.agents/skills/<name>/SKILL.md` (one level deep only) and invoked as `/<name>`.
