...
<1-2 paragraphs: build order and why. Note which phases/tasks are parallelizable (waves) for fan-out — a wave is valid only when every pair of its tasks passes `agent-delegation.md`'s disjointness test (no file/module overlap, no output dependency, no ordering dependency).>
...
2. **Pick** the first runnable pending task whose dependencies are satisfied. Skip tasks marked `⚠ blocked` and invalid legacy struck-unticked rows; they are not runnable and keep their phase incomplete. Do not stop at the roadmap's author-time wave markings: each iteration, re-check the set of currently-runnable pending tasks against `agent-delegation.md`'s disjointness test — a marking can go stale as tasks tick or get replanned, and an unmarked pair can still be genuinely disjoint. Fan out the resulting disjoint set in one message; for a wave that edits code, follow `agent-delegation.md`'s Worktree Lifecycle for Parallel Coding Work end to end (isolated spawn, one-at-a-time merge, re-verify on the merged state, confirm the merge landed, delete the worktree only after that).
