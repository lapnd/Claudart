---
paths: ["**/*"]
description: Evidence-first proof discipline for code changes — tier calibration by blast radius, observed-red-before-green ordering, a gauntlet of executable layers (changed-line coverage, mutation, property-based tests), absolute anti-gaming rules, and fail-closed checkers. Supplies the proof half that a coverage bar assumes.
when_to_use: When a change needs proof rather than assertion — a task or roadmap item that names a tier, any bug fix, any security- or money-sensitive surface, or an invocation of $codex-prove. Read before writing the first test, not after the implementation.
tags: [testing, evidence, tdd, mutation, verification]
---

# Evidence Gauntlet

A coverage bar (≥95%, behavior not implementation) states the standard. This guideline is how that bar is **proved rather than asserted**. Its premise: a reviewer who reads every line does not scale to agent-written code, so trust has to move from inspection to constraints the author cannot quietly weaken.
...
**Global percentage is context, changed-line coverage is the gate.** The ≥95% bar remains the project standard; it is a detector of drift across the codebase. What blocks this change is whether the lines this change touched are exercised.

A gauntlet layer is enforceable only when it is **named in an acceptance scenario or a `verify:` line**. `.codex/guidelines/spec-workflow.md` forbids inferring a defect from broad language like "quality" or "production-ready" — so an unnamed layer is advisory, and naming it is what makes it binding.

## 4. Anti-gaming rules (absolute)

The gauntlet creates trust only if it cannot be gamed. These are hard rules, extending `.codex/guidelines/ai-behavior.md` §5.
...
Evidence follows the anatomy already defined in `.codex/guidelines/spec-workflow.md` — the claim, the exact command, 1-3 decisive output lines, and the revision observed at. This guideline adds one event and two obligations:
...
Commits during the loop are **not** this guideline's grant. Cadence routes through the existing policy — a spec's `commits:` frontmatter (`user` | `per-task` | `per-phase`) or the user's explicit request — under the repository commit conventions. Never impose a commit cadence on a repo whose owner has not agreed to one.
