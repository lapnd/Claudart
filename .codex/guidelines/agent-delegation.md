...
When a disjoint-task wave (per the test above) involves real code edits, spawn each writer in its own isolated worktree so concurrent edits cannot conflict. This section is what happens after they return — a worktree is not done just because the worker said so.

1. **Spawn.** Fan out one subagent call per disjoint task in a single message, each in its own isolated worktree. The worker prompt follows the standard Worker Prompt Contract; its **Output** must additionally echo the worktree path and branch it worked in, as a cross-check against what the harness reports.
2. **Collect.** For each finished worker, take the worktree path and branch from the harness result (an isolated worktree spawn returns them whenever the agent made changes). A worktree with no changes was already auto-cleaned — nothing further to do for it.
...
6. **Delete the worktree only after merge (and push, if any) is confirmed.** Use `git worktree remove <path>` (and `git branch -d <branch>` once merged) — never a session-scoped worktree-exit tool, which is scoped to worktrees the _current session_ itself entered and is a no-op on a worktree a subagent created through isolated spawn.
7. **Never delete a worktree still holding unmerged or unconfirmed work.** A merge conflict or a failed re-verify keeps the worktree and its branch on disk; treat it like any other blocked unit under the owning workflow's existing blocked/circuit-breaker handling (`spec-workflow.md`'s convergence rules for spec work, or a direct report for `$codex-plan` work) rather than inventing new failure handling or discarding the branch.
