  ...
  awk -v sep="$SEP" -v meta="$parse_meta" -v diag="$parse_diag" -v kind="$parse_kind" '
    ...
    function known(key) {
      return key == "name" || key == "description" || key == "type" ||
        ...
        key == "generated" || key == "verified" || key == "stale_after"
    }
    function list_key(key) {
      return key == "aliases" || key == "triggers" || key == "scope" ||
        key == "related" || key == "supersedes"
    }
    function maplist_key(key) {
      return key == "sources" || key == "verified"
    }
    ...
    function valid_instant(value, datepart) {
      if (value !~ /^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9](\.[0-9]+)?(Z|[+-][0-9][0-9]:[0-9][0-9])$/) {
        return 0
      }
      datepart = substr(value, 1, 10)
      return valid_date(datepart)
    }
    function unwrap(value) {
      ...
      if (value !~ "^[A-Za-z0-9][A-Za-z0-9._:/+-]*$") return ""
      return value
    }
    function actor(value) {
      return value ~ "^[A-Za-z0-9][A-Za-z0-9._+-]*[:/][A-Za-z0-9][A-Za-z0-9._:/+-]*$"
    }
    function split_pair(text, colonpos) {
      pair_key = ""
      pair_value = ""
      colonpos = index(text, ":")
      if (colonpos < 2) return 0
      pair_key = substr(text, 1, colonpos - 1)
      if (pair_key !~ /^[A-Za-z_][A-Za-z0-9_-]*$/) return 0
      if (substr(text, colonpos, 2) != ": ") return 0
      pair_value = substr(text, colonpos + 2)
      return pair_value != ""
    }
    function source_item(text, line, decoded_source) {
      if (!split_pair(text) || pair_key != "resource") {
        error("K102", line,
          "sources item must be a one-key mapping under the resource key")
        return
      }
      if (!quoted(pair_value)) {
        error("K102", line,
          "sources resource must be one-line double-quoted text")
        return
      }
      decoded_source = decode(pair_value)
      if (decoded_source == "") {
        error("K106", line, "list item must not be empty")
        return
      }
      list_count["sources"]++
      print "L" sep "sources" sep decoded_source sep line >> meta
    }
    function close_verified_entry() {
      if (verified_open && !verified_at_seen) {
        error("K110", verified_open_line,
          "verified entry requires an at instant")
      }
      verified_open = 0
    }
    function verified_item(text, line, decoded_actor) {
      close_verified_entry()
      if (!split_pair(text) || pair_key != "by") {
        error("K102", line, "verified item must start with a by actor")
        return
      }
      decoded_actor = unwrap(pair_value)
      if (!actor(decoded_actor)) {
        error("K106", line, "verified by must be a typed actor token")
        return
      }
      list_count["verified"]++
      verified_open = 1
      verified_open_line = line
      verified_at_seen = 0
      if (substr(decoded_actor, 1, 6) == "human:") verified_human++
    }
    function verified_field(text, line, decoded_instant) {
      if (!split_pair(text) || pair_key != "at") {
        error("K102", line, "verified entry supports only an at instant")
        return
      }
      if (verified_at_seen) {
        error("K104", line, "duplicate metadata key: at")
        return
      }
      decoded_instant = unwrap(pair_value)
      if (!valid_instant(decoded_instant)) {
        error("K108", line, "verified at must be an absolute ISO instant")
        return
      }
      verified_at_seen = 1
    }
    function generated_field(text, line, decoded_value) {
      if (!split_pair(text)) {
        error("K102", line, "generated mapping requires by and at keys")
        return
      }
      decoded_value = unwrap(pair_value)
      if (pair_key == "by") {
        if (generated_by_seen) {
          error("K104", line, "duplicate metadata key: by")
          return
        }
        if (!actor(decoded_value)) {
          error("K106", line, "generated by must be a typed actor token")
          return
        }
        generated_by_seen = 1
      } else if (pair_key == "at") {
        if (generated_at_seen) {
          error("K104", line, "duplicate metadata key: at")
          return
        }
        if (!valid_instant(decoded_value)) {
          error("K108", line, "generated at must be an absolute ISO instant")
          return
        }
        generated_at_seen = 1
      } else {
        error("K103", line, "unknown metadata key: generated." pair_key)
      }
    }
    function finish_list() {
      ...
      if (current_maplist != "" && list_count[current_maplist] == 0) {
        error("K114", list_line[current_maplist],
          "empty list must be omitted instead of declared")
      }
      if (current_map != "" && !generated_by_seen && !generated_at_seen) {
        error("K114", list_line[current_map],
          "empty mapping must be omitted instead of declared")
      }
      current_list = ""
      current_maplist = ""
      current_map = ""
    ...
    function scalar(key, value, line) {
      if (free_text_key(key)) {
        ...
        if (decoded == "" || !valid_instant(decoded)) {
          error("K108", line, "stale_after must be an absolute ISO instant")
          return
        }
        scalar_value[key] = decoded
      } else {
    ...
    function validate_contract( required, i, key, value, status, type) {
      ...
      if (kind == "map") {
        if (seen["status"] && status != "stable" && status != "draft") {
          error("K111", scalar_line["status"],
            "map status must be stable or draft")
        }
      } else if (seen["status"] &&
          status != "stable" && status != "draft" && status != "deprecated") {
      ...
      if (seen["generated"] && (generated_by_seen || generated_at_seen) &&
          (!generated_by_seen || !generated_at_seen)) {
        error("K105", list_line["generated"],
          "generated mapping requires both by and at")
      }
      if (status == "stable") {
        if (!seen["last_verified"]) {
          error("K109", scalar_line["status"],
            "stable entry requires last_verified")
        }
        if (list_count["sources"] == 0 && !seen["verify"]) {
          error("K109", scalar_line["status"],
            "stable entry requires at least sources or verify")
        }
        if (verified_human == 0) {
          error("K109", scalar_line["status"],
            "stable entry requires a verified entry with a human actor")
        }
      } else if (status == "draft" || status == "deprecated") {
        if (!seen["status_note"]) {
          error("K110", scalar_line["status"],
            "non-stable entry requires a double-quoted status_note")
        }
      }
    }
    NR == 1 {
    ...
    }
    {
      ...
      if ($0 ~ /^  - /) {
        if (current_maplist == "sources") {
          source_item(substr($0, 5), NR)
          next
        }
        if (current_maplist == "verified") {
          verified_item(substr($0, 5), NR)
          next
        }
        if (current_list == "") {
      ...
      if ($0 ~ /^    [A-Za-z_][A-Za-z0-9_-]*: /) {
        if (current_maplist == "verified" && verified_open) {
          verified_field(substr($0, 5), NR)
          next
        }
        error("K102", NR,
      ...
      if ($0 ~ /^  [A-Za-z_][A-Za-z0-9_-]*: /) {
        if (current_map == "generated") {
          generated_field(substr($0, 3), NR)
          next
        }
        error("K102", NR,
          "unsupported list indentation or nested YAML structure")
      ...
      if (maplist_key(key)) {
        if ($0 != key ":") {
          error("K102", NR,
            "list must use a bare key and two-space block mapping items")
          next
        }
        current_maplist = key
        list_line[key] = NR
      ...
      if (map_key(key)) {
        if ($0 != key ":") {
          error("K102", NR,
            "generated must use a bare key and two-space by and at items")
          next
        }
        current_map = key
        list_line[key] = NR
...
scan_artifact() {
  ...
  if [ "$artifact_status" = stable ] && [ -n "$artifact_verified" ]; then
...
validate_source() {
  ...
  if [ "$GIT_READY" -eq 1 ] && [ "$source_status" = stable ] &&
...
if [ "$direct_count" -gt 24 ]; then
  add_finding WARN K211 "$INDEX_REL" 1 \
    "root has more than 24 stable direct topic routes without maps"
fi
    ...
    stable)
      ...
      if [ "$reachable_count" -ne 1 ]; then
        add_finding ERROR K205 "$valid_rel" "$(meta_line "$valid_id" status)" \
          "stable entry must be reachable from the root exactly once"
      fi
      ;;
    draft)
      if [ "$valid_kind" = map ]; then
      ...
      if [ "$reachable_count" -eq 0 ]; then
        add_finding WARN K206 "$valid_rel" "$(meta_line "$valid_id" status)" \
          "draft entry is intentionally not auto-promoted and has no route"
      elif [ "$reachable_count" -gt 1 ]; then
        add_finding ERROR K207 "$valid_rel" "$(meta_line "$valid_id" status)" \
          "draft entry may be reachable at most once"
      fi
      ;;
    deprecated)
      ...
      if [ "$active_hits" -gt 0 ]; then
        add_finding ERROR K208 "$valid_rel" "$(meta_line "$valid_id" status)" \
          "deprecated entry must not be routed as stable"
      fi
      ;;
  esac
done <"$VALID"
