---
name: deepseek-start
description: Orient a new DeepSeek session from current state, active task/spec indexes, the root knowledge router, and recent Git history without running heavy validation.
---

# DeepSeek Start

Start a DeepSeek session with a lightweight CLAUDART orientation. This skill is read-only, with one exception: consuming the handoff baton (Case H) deletes `.deepseek/HANDOFF.md` once the user resumes or discards it.
...
### Case H: `.deepseek/HANDOFF.md` exists (a previous session handed off mid-flight)
...
### Case S: `.deepseek/specs/INDEX.md` lists an Active spec (mission in flight)
...
- `awaiting-review`: a previous session reported the task complete and is waiting for the user's verification. Say:
  > "Task `<slug>` is `awaiting-review` — a previous session finished it and is waiting for your verification. Open `.deepseek/tasks/<file>` to review draft Outcomes. Confirm to close, or tell me what didn't work and I'll flip it back to `in-progress`."
- `in-progress`: say:
