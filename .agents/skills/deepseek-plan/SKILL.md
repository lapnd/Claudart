---
name: deepseek-plan
description: Create a persistent implementation plan as a markdown document in .deepseek/tasks/. Replaces session-only /plan mode with a cross-session task file DeepSeek maintains until completion.
---

# DeepSeek Plan

Create a persistent task document. The file you produce — not this DeepSeek session — is the source of truth for the plan. A future session must be able to resume work from that file alone.

Before doing anything, read `.deepseek/guidelines/task-management.md`. That guideline defines the file schema, status state machine, planning lock, approval signals, progress update protocol, and completion flow. Also read `.deepseek/guidelines/agent-delegation.md` if it exists, because large tasks may need a delegation strategy. This skill does not duplicate those contracts; it only orchestrates creation.
...
- Read `.deepseek/CONTEXT.md` (current state of work).
- Read `.deepseek/tasks/index.md` if it exists. If an active task already covers this request, surface it and ask whether to continue that file instead of starting a new one.
- Read `.deepseek/knowledge/INDEX.md`, then route only relevant maps/topics within `.deepseek/guidelines/knowledge-management.md`'s bounds.
...
You are in planning lock from this point. Do not write implementation code. Normally write only the task file and `index.md`; the triggered knowledge-maintenance exception in `.deepseek/guidelines/task-management.md` still applies.
...
- If the task may parallelize, record a delegation _strategy_ (decomposition, ownership) in the `delegation:` field + Plan of Work — the field's semantics live in `.deepseek/guidelines/agent-delegation.md`. Write-scope subagents must wait for `in-progress`; read-only explorers are fine during the planning lock.
...
- Sequence number: scan `.deepseek/tasks/` and `.deepseek/tasks/done/` for existing files whose name starts with today's UTC date (`YYYY-MM-DD-`). The next sequence number is the highest existing NNN for that date + 1, zero-padded to 3 digits (e.g. `001`, `002`). If no files exist for today, start at `001`.
- Filename: `YYYY-MM-DD-NNN-<slug>.md` using today's UTC date and the computed sequence number.
- Path: `.deepseek/tasks/<filename>`.
...
Use the exact skeleton in `.deepseek/guidelines/task-management.md`. Fill every section:

- **Frontmatter**: `status: planning`, today's date in `created` and `updated`, `agent: deepseek`, `delegation:` (`none` | `strategy-only` | `authorized` — semantics per `.deepseek/guidelines/agent-delegation.md`; record the choice in the Decision Log), 1-5 lowercase kebab tags.
...
If `.deepseek/tasks/index.md` does not exist, create it with the canonical header (see guideline file). Add the new task under `## Active`:
...
Do not begin implementing. Wait for the approval signal defined in `.deepseek/guidelines/task-management.md`.

## After Approval

Once the user gives an approval signal, the contract in `.deepseek/guidelines/task-management.md` takes over — this skill adds nothing to it. Flip `status: planning -> in-progress`, execute Concrete Steps while maintaining the task file per "Progress Updates During Implementation", honor the `delegation:` field per `.deepseek/guidelines/agent-delegation.md`, and finish through the **Two-Phase Completion Gate**: report at `awaiting-review` and stop; the user — not you — confirms `done`.

## Anti-Patterns

- Do not write code while `status: planning` or `status: awaiting-review` — both are read-only locks (see `.deepseek/guidelines/task-management.md`).
