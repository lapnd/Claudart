---
name: deepseek-learn
description: Run a behavior-first DeepSeek retrospective, strengthen relevant guidelines, and route any durable descriptive facts to the canonical knowledge workflow.
---
...
3. Read `.deepseek/CONTEXT.md` for current shared state.
4. If the retrospective may retrieve or mutate project knowledge, read `.deepseek/guidelines/knowledge-management.md` in full. Otherwise do not load it.
5. For recurrence evidence, tail at most ~200 lines of `.deepseek/JOURNAL.md`, then use targeted `rg` for matching decisions or pivots. Never full-read JOURNAL by default.
...
3. Resolve contradictions between guidelines or `DEEPSEEK.md` immediately when evidence identifies the winner. If neither wins, surface the decision instead of silently choosing.
4. Use an existing guideline only when the semantic match is at least 80%. Create a focused owner instead of polluting a weak match; add canonical frontmatter and reference it from `DEEPSEEK.md` when globally relevant.
5. Put delegation behavior in `.deepseek/guidelines/agent-delegation.md`.
6. Put a repository-wide DeepSeek standard in `DEEPSEEK.md`.
...
- Descriptive, durable-beyond-current-work, current, evidenced fact → follow the full knowledge guideline now. Patch the existing topic owner first, update its reachable route atomically, and run `bash .deepseek/scripts/knowledge-check.sh --root .` after the mutation.
...
- Do not modify `.deepseek/CONTEXT.md`; checkpoint owns it.
- Do not rewrite or delete `.deepseek/JOURNAL.md`.
