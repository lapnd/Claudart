---
name: pi-refactor-memory
description: Consolidate Pi project memory and perform controlled, in-place normalization of knowledge, guidelines, skills, and agent wiring.
---

# Pi Refactor Memory

Refactor the repository-local Pi operating layer without losing project context or curated routing. Use the existing files as live state, make deterministic repairs before semantic ones, and preserve user-authored bodies unless evidence supports an explicit content change.

For a knowledge contract upgrade, use this sequence:

```text
/skill:pi-doctor
/skill:pi-refactor-memory
/skill:pi-doctor
```
...
2. Resolve the active memory index: installed projects normally use root `PI.md`; the CLAUDART source template may use `.pi/PI.md` as the installer source. If both differ and local evidence does not identify the owner, ask before overwriting either.
3. Inventory `.pi/CONTEXT.md`, `.pi/JOURNAL.md`, guidelines, knowledge, tasks, specs, skills, agents, and recent Git history. Never read `.env` or expose secrets.
4. Read `.pi/guidelines/knowledge-management.md` in full before inspecting or changing knowledge.
5. Require `.pi/scripts/knowledge-check.sh`. If it is missing, report a High-severity installation problem and stop before knowledge writes.
6. Run `bash .pi/scripts/knowledge-check.sh --root .` and retain its pre-change findings. Exit `1` is a finding baseline; exit `2` is a checker usage/runtime failure, so stop before knowledge writes.

## 2. Keep The Memory Tiers Distinct

- `PI.md`: concise entrypoint and routing pointers, preferably under 100 lines.
- `.pi/guidelines/`: durable prescriptive behavior.
- `.pi/knowledge/`: durable descriptive project facts and reference pointers.
- `.pi/CONTEXT.md`: small declarative state true now.
- `.pi/JOURNAL.md`: append-only history, never auto-loaded.
- `.pi/tasks/` and `.pi/specs/`: working plans, proposals, acceptance state, and mission-local discoveries.
- `.pi/HANDOFF.md`: optional single-use conversational baton.
- `.agents/skills/`: executable workflows. The Pi layer ships no specialist agent files.
...
3. Discover linters, formatters, test runners, and validation commands. Delegate style enforcement to those tools instead of copying their rules into `PI.md`.
4. Inspect the active `PI.md`, deprecated memory files, current guidelines, and stable decisions in CONTEXT. Split candidates by type before moving them: behavior → guideline; fact → knowledge; WIP/proposal → task/spec/CONTEXT.
5. Ensure `.pi/guidelines/` exists, then extract detailed behavior into the smallest set of domain guidelines with clear ownership and useful `paths:`. Do not create files for symmetry or force a weak concept into an unrelated owner. Use `.pi/guidelines/*.md` for semantic guidance; never place it in `.pi/rules/`, whose optional `*.rules` files are reserved for Pi permission or environment rules.
...
1. Trim the active `PI.md` to project identity, selective context loading, core workflows, security/repository-wide constraints, and pointers. Never require reading every guideline blindly. Keep it under 100 lines where practical; if it still exceeds 150 lines, flag that explicitly in the final report.
...
3. Keep the four compact knowledge invariants in `PI.md` and the full contract in `knowledge-management.md`.
...
8. Cross-link `.pi/CONTEXT.md`, the universal behavior guideline, every globally relevant workflow guideline, and the knowledge root router from `PI.md`. Never auto-load JOURNAL, HANDOFF, task bodies, or knowledge details.
...
10. Ensure `## Agent Self-Evolution & Context Maintenance` remains in `PI.md`: project-wide behavior updates its owner guideline, new guideline owners get indexed, eligible descriptive facts use the knowledge contract, global Pi behavior updates `PI.md`, and live state uses checkpoint.
...
- Treat `.pi/` and `.agents/` as generic payload, not live maintainer state.
- Preserve the documented relationship between `.pi/PI.md` and the downstream root `PI.md`.
...
4. Confirm `PI.md` line count and links; verify live state and guideline targets exist and that JOURNAL, HANDOFF, task bodies, and knowledge details are not auto-loaded.
...
8. Confirm the delegation guideline still states that Pi has no built-in subagents; never add a shipped `.pi/config.toml` or `.pi/agents/` — `.pi/settings.json` and `.pi/agents/skills` belong to Pi itself.
