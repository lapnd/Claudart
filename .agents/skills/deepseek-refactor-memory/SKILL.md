---
name: deepseek-refactor-memory
description: Consolidate DeepSeek project memory and perform controlled, in-place normalization of knowledge, guidelines, skills, and agent wiring.
---

# DeepSeek Refactor Memory

Refactor the repository-local DeepSeek operating layer without losing project context or curated routing. Use the existing files as live state, make deterministic repairs before semantic ones, and preserve user-authored bodies unless evidence supports an explicit content change.
...
2. Resolve the active memory index: installed projects normally use root `DEEPSEEK.md`; the CLAUDART source template may use `.deepseek/DEEPSEEK.md` as the installer source. If both differ and local evidence does not identify the owner, ask before overwriting either.
3. Inventory `.deepseek/CONTEXT.md`, `.deepseek/JOURNAL.md`, guidelines, knowledge, tasks, specs, skills, agents, and recent Git history. Never read `.env` or expose secrets.
4. Read `.deepseek/guidelines/knowledge-management.md` in full before inspecting or changing knowledge.
5. Require `.deepseek/scripts/knowledge-check.sh`. If it is missing, report a High-severity installation problem and stop before knowledge writes.
6. Run `bash .deepseek/scripts/knowledge-check.sh --root .` and retain its pre-change findings. Exit `1` is a finding baseline; exit `2` is a checker usage/runtime failure, so stop before knowledge writes.

## 2. Keep The Memory Tiers Distinct

- `DEEPSEEK.md`: concise entrypoint and routing pointers, preferably under 100 lines.
- `.deepseek/guidelines/`: durable prescriptive behavior.
- `.deepseek/knowledge/`: durable descriptive project facts and reference pointers.
- `.deepseek/CONTEXT.md`: small declarative state true now.
...
1. Trim the active `DEEPSEEK.md` to project identity, selective context loading, core workflows, security/repository-wide constraints, and pointers. Never require reading every guideline blindly. Keep it under 100 lines where practical; if it still exceeds 150 lines, flag that explicitly in the final report.
...
3. Keep the four compact knowledge invariants in `DEEPSEEK.md` and the full contract in `knowledge-management.md`.
...
8. Cross-link `.deepseek/CONTEXT.md`, the universal behavior guideline, every globally relevant workflow guideline, and the knowledge root router from `DEEPSEEK.md`. Never auto-load JOURNAL, HANDOFF, task bodies, or knowledge details.
...
10. Ensure `## Agent Self-Evolution & Context Maintenance` remains in `DEEPSEEK.md`: project-wide behavior updates its owner guideline, new guideline owners get indexed, eligible descriptive facts use the knowledge contract, global DeepSeek behavior updates `DEEPSEEK.md`, and live state uses checkpoint.
...
- Treat `.deepseek/` and `.agents/` as generic payload, not live maintainer state.
- Preserve the documented relationship between `.deepseek/DEEPSEEK.md` and the downstream root `DEEPSEEK.md`.
...
4. Confirm `DEEPSEEK.md` line count and links; verify live state and guideline targets exist and that JOURNAL, HANDOFF, task bodies, and knowledge details are not auto-loaded.
