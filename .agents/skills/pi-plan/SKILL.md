---
name: pi-plan
description: Create a persistent implementation plan as a markdown document in .pi/tasks/. Replaces session-only /plan mode with a cross-session task file Pi maintains until completion.
---

# Pi Plan

Create a persistent task document. The file you produce — not this Pi session — is the source of truth for the plan. A future session must be able to resume work from that file alone.

Before doing anything, read `.pi/guidelines/task-management.md`. That guideline defines the file schema, status state machine, planning lock, approval signals, progress update protocol, and completion flow. Also read `.pi/guidelines/agent-delegation.md` if it exists, because large tasks may need a delegation strategy. This skill does not duplicate those contracts; it only orchestrates creation.

## Inputs

- The user's request after `/skill:pi-plan` is the task description. If invoked with no argument, ask one short clarifying question: "What's the task?"
...
Skip `/skill:pi-plan` and answer in chat if any of these apply:
...
Use `/skill:pi-plan` when the work crosses sessions, touches multiple files, requires decisions worth recording, or the user explicitly asked for one.

### Step 2: Read project context

- Read `.pi/CONTEXT.md` (current state of work).
- Read `.pi/tasks/index.md` if it exists. If an active task already covers this request, surface it and ask whether to continue that file instead of starting a new one.
- Read `.pi/knowledge/INDEX.md`, then route only relevant maps/topics within `.pi/guidelines/knowledge-management.md`'s bounds.
...
You are in planning lock from this point. Do not write implementation code. Normally write only the task file and `index.md`; the triggered knowledge-maintenance exception in `.pi/guidelines/task-management.md` still applies.
...
- If the task may parallelize, record a delegation _strategy_ (decomposition, ownership) in the `delegation:` field + Plan of Work — the field's semantics live in `.pi/guidelines/agent-delegation.md`. Write-scope subagents must wait for `in-progress`; read-only explorers are fine during the planning lock.
...
- Sequence number: scan `.pi/tasks/` and `.pi/tasks/done/` for existing files whose name starts with today's UTC date (`YYYY-MM-DD-`). The next sequence number is the highest existing NNN for that date + 1, zero-padded to 3 digits (e.g. `001`, `002`). If no files exist for today, start at `001`.
- Filename: `YYYY-MM-DD-NNN-<slug>.md` using today's UTC date and the computed sequence number.
- Path: `.pi/tasks/<filename>`.
...
Use the exact skeleton in `.pi/guidelines/task-management.md`. Fill every section:

- **Frontmatter**: `status: planning`, today's date in `created` and `updated`, `agent: pi`, `delegation:` (`none` | `strategy-only` | `authorized` — semantics per `.pi/guidelines/agent-delegation.md`; record the choice in the Decision Log), 1-5 lowercase kebab tags.
...
- **Context & Orientation**: this is the handoff to future-self. Fill all three subsections:
  ...
  - _Memory Hints_: free-form notes — non-obvious task/WIP context and uncertain discoveries that a fresh agent would otherwise re-discover. Keep these as candidates by default. A claim may move to knowledge immediately only under the task guideline's exception and the full capture gate in `knowledge-management.md`; patch the owner plus route atomically and run the checker. `/skill:pi-checkpoint` bulk-evaluates remaining candidates; `/skill:pi-learn` owns behavior, not fact promotion.
...
If `.pi/tasks/index.md` does not exist, create it with the canonical header (see guideline file). Add the new task under `## Active`:
...
Do not begin implementing. Wait for the approval signal defined in `.pi/guidelines/task-management.md`.

## After Approval

Once the user gives an approval signal, the contract in `.pi/guidelines/task-management.md` takes over — this skill adds nothing to it. Flip `status: planning -> in-progress`, execute Concrete Steps while maintaining the task file per "Progress Updates During Implementation", honor the `delegation:` field per `.pi/guidelines/agent-delegation.md`, and finish through the **Two-Phase Completion Gate**: report at `awaiting-review` and stop; the user — not you — confirms `done`.

## Anti-Patterns

- Do not write code while `status: planning` or `status: awaiting-review` — both are read-only locks (see `.pi/guidelines/task-management.md`).
