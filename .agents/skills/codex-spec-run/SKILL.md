...
The loop is model-agnostic: run it on a cheap model for routine execution, and run the _same skill_ from a stronger session to punch through a `blocked` spec — escalation is not a different workflow (see the guideline file's unblock mode). SPEC frontmatter `executor-tier:` records the cheapest recommended session tier (see `agent-delegation.md` → Model & Effort Routing); if this session is below it, say so before starting.
...
- A second `validation-failed` on the same task in a session below the strong tier is an escalation signal: before a third attempt, offer rotation to a stronger session — escalation is this same skill from that session — unless the next attempt is materially different for reasons unrelated to reasoning depth.
